...
wp_die(wp_kses_post(
'<strong>' . __('Plugin activation is failed because the required database tables could not created!', 'review-widgets-for-tripadvisor') . '</strong><br /><br />' .
/* translators: %s: database */
sprintf(__('We got the following error from %s:', 'review-widgets-for-tripadvisor'), __('database', 'review-widgets-for-tripadvisor')) .
'<pre style="'. $preStyle .'">'. $mysqlError .'</pre>' .
'<strong>' . __('Run the following SQL codes in your database administration interface (e.g. PhpMyAdmin) to create the tables or contact your system administrator:', 'review-widgets-for-tripadvisor') . '</strong>' .
'<pre style="'. $preStyle .'">' . implode('</pre><pre style="'. $preStyle .'">', $sqlsToRun) . '</pre>' .
'<strong>' . __('Then try activate the plugin again.', 'review-widgets-for-tripadvisor') . '</strong>'
));
}
