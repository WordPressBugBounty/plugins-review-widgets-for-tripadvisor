...
/* translators: %s: URL */
echo wp_kses_post($pluginManagerInstance->frontEndErrorForAdmins(sprintf(__("Please fill out <strong>all the required fields</strong> in the <a href='%s'>widget settings</a> page", 'review-widgets-for-tripadvisor'), admin_url('admin.php?page='.$pluginManagerInstance->get_plugin_slug().'/settings.php'))));
}
...
esc_html(__('You can only list 10 reviews without it.', 'review-widgets-for-tripadvisor')) . '<br>'
/* translators: %s: URL */
. wp_kses_post(sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'review-widgets-for-tripadvisor'), admin_url('admin.php?page='.$pluginManagerInstance->get_plugin_slug().'/settings.php&tab=setup_trustindex')))
)); ?>
<?php endif; ?>
</div>
<?php
