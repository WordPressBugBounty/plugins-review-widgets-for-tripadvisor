<?php
class TrustindexPlugin_tripadvisor
{
...
'name' => __('Free Widget Configurator', 'review-widgets-for-tripadvisor')
];
if ($this->is_noreg_linked()) {
$tabs[] = [
'place' => 'left',
'slug' => 'my-reviews',
'name' => __('My reviews', 'review-widgets-for-tripadvisor')
];
}
$tabs[] = [
'place' => 'left',
'slug' => 'get-reviews',
'name' => __('Get Reviews', 'review-widgets-for-tripadvisor')
];
$tabs[] = [
'place' => 'left',
'slug' => 'rate-us',
'name' => __('Rate Us', 'review-widgets-for-tripadvisor')
];
if (!$this->is_trustindex_connected()) {
$tabs[] = [
'place' => 'left',
'slug' => 'get-more-features',
'name' => __('Get more Features', 'review-widgets-for-tripadvisor')
];
}
$tabs[] = [
'place' => 'right',
...
'place' => 'right',
'slug' => 'advanced',
'name' => __('Advanced', 'review-widgets-for-tripadvisor')
];
return $tabs;
}
...
return '<a class="ti-btn" href="https://lp.trustindex.io/'.$this->getShortName().'-wp/?a=sys&c='. $campaignId .'" target="_blank">'. __('Create a Free Account for More Features', 'review-widgets-for-tripadvisor') .'</a>';
}
...
$settingsPageTitle .= mb_strtolower(__('Reviews', 'review-widgets-for-tripadvisor'));
}
else {
$settingsPageTitle .= strtolower(__('Reviews', 'review-widgets-for-tripadvisor'));
}
$topMenu = false;
...
$platformLink .= 'href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=my-reviews') . '">'. __('Review Management', 'review-widgets-for-tripadvisor');
}
else {
$platformLink .= 'href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php') . '">';
if (!$this->is_noreg_linked()) {
/* translators: %s: Platform name */
$platformLink .= sprintf(__('Connect %s', 'review-widgets-for-tripadvisor'), $this->platform_name);
}
else {
$platformLink .= __('Create Widget', 'review-widgets-for-tripadvisor');
}
}
$platformLink .= '</a>';
$settingsLink = '<a href="' . admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php') . '">' . __('Settings', 'review-widgets-for-tripadvisor') . '</a>';
...
{
if (basename($file) === basename($this->plugin_file_path)) {
$meta[] = '<a href="'. admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=get-more-features') .'">'. __('Get more Features', 'review-widgets-for-tripadvisor') . ' →</a>';
$meta[] = '<a href="http://wordpress.org/support/view/plugin-reviews/'. $this->get_plugin_slug() .'" target="_blank" rel="noopener noreferrer">'. __('Rate our plugin', 'review-widgets-for-tripadvisor') . ' <span style="color: #F6BB07; font-size: 1.2em; line-height: 1; position: relative; top: 0.05em;">★★★★★</span></a>';
}
return $meta;
}
...
sprintf(__('We have worked a lot on the free "%s" plugin.', 'review-widgets-for-tripadvisor'), $this->plugin_name) . '<br />' .
__('If you love our features, please write a review to help us make the plugin even better.', 'review-widgets-for-tripadvisor') . '<br />' .
/* translators: %s: Trustindex CEO */
sprintf(__('Thank you. Gabor, %s', 'review-widgets-for-tripadvisor'), 'Trustindex CEO'),
],
'not-using-no-connection' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Platform name */
'button-text' => sprintf(__('Create a free %s widget! »', 'review-widgets-for-tripadvisor'), $platformName),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
...
/* translators: %s: Platform name */
'text' => sprintf(__('Display %s reviews on your website.', 'review-widgets-for-tripadvisor'), $platformName),
],
'not-using-no-widget' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Platform name */
'button-text' => sprintf(__('Embed the %s reviews widget! »', 'review-widgets-for-tripadvisor'), $platformName),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
...
'redirect' => $defaultRedirect,
/* translators: %s: Platform name */
'text' => sprintf(__('Build trust and display your %s reviews on your website.', 'review-widgets-for-tripadvisor'), $platformName),
],
'review-download-available' => [
'type' => 'warning',
'extra-class' => "",
'button-text' => __('Download your latest reviews! »', 'review-widgets-for-tripadvisor'),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
'remind-later-button' => false,
...
/* translators: %s: Platform name */
'text' => sprintf(__('You can update your %s reviews.', 'review-widgets-for-tripadvisor'), $platformName),
],
'review-download-finished' => [
'type' => 'warning',
'extra-class' => "",
/* translators: %s: Service name (ChatGPT) */
'button-text' => sprintf(__('Reply with %s! »', 'review-widgets-for-tripadvisor'), 'ChatGPT'),
'is-closeable' => true,
'hide-on-close' => true,
'hide-on-open' => true,
'remind-later-button' => false,
'redirect' => '?page='. $this->get_plugin_slug() .'/settings.php&tab=my-reviews',
/* translators: %s: Platform name */
'text' => sprintf(__('Your new %s reviews have been downloaded.', 'review-widgets-for-tripadvisor'), $platformName),
],
];
...
$chosedPlatform = new $className($forcePlatform, $filePath, "do-not-care-13.2.7", "do-not-care-WP Tripadvisor Review Widgets", "do-not-care-Tripadvisor");
$chosedPlatform->setNotificationParam('not-using-no-widget', 'active', false);
if (!$chosedPlatform->is_noreg_linked()) {
/* translators: %s: Platform name */
return wp_kses_post($this->frontEndErrorForAdmins(sprintf(__('You have to connect your business (%s)!', 'review-widgets-for-tripadvisor'), $forcePlatform)));
...
return wp_kses_post($this->frontEndErrorForAdmins(__('Your shortcode is deficient: Trustindex Widget ID is empty! Example: ', 'review-widgets-for-tripadvisor') . '<br /><code>['.$this->get_shortcode_name().' data-widget-id="478dcc2136263f2b3a3726ff"]</code>'));
}
}
...
return self::get_alertbox('error', ' @ <strong>'. __('Trustindex plugin', 'review-widgets-for-tripadvisor') .'</strong> <i style="opacity: 0.65">('. __('This message is not be visible to visitors in public mode.', 'review-widgets-for-tripadvisor') .')</i><br /><br />'. $text, false);
}
...
<p>' .
'<strong>'. __('ERROR with the following plugin:', 'review-widgets-for-tripadvisor') .'</strong> '. $this->plugin_name .'<br /><br />' .
__('CSS file could not saved.', 'review-widgets-for-tripadvisor') .' <strong>('. $this->getCssFile() .')</strong> '. __('Your widgets do not display properly!', 'review-widgets-for-tripadvisor') . '<br />';
...
if ($fileExists) {
$html .= __('CSS file exists and it is not writeable. Delete the file', 'review-widgets-for-tripadvisor');
}
else {
$html .= __('Grant write permissions to upload folder', 'review-widgets-for-tripadvisor');
}
$html .= '<br />' .
__('or', 'review-widgets-for-tripadvisor') . '<br />' .
/* translators: %s: Advanced page link */
sprintf(__("enable 'CSS internal loading' in the %s page!", 'review-widgets-for-tripadvisor'), '<a href="'. admin_url('admin.php?page=' . $this->get_plugin_slug() . '/settings.php&tab=advanced') .'>'. __('Advanced', 'review-widgets-for-tripadvisor') .'</a>');
}
echo wp_kses($html, [
'p' => [], 'strong' => [], 'br' => [],
...
return $this->frontEndErrorForAdmins(__('Free plugin features are unavailable with AMP plugin.', 'review-widgets-for-tripadvisor'));
}
...
$text = sprintf(__('Our exclusive "Top Rated" badge is awarded to service providers with a rating of %s and above.', 'review-widgets-for-tripadvisor'), self::$topRatedMinimumScore)
.'<br />'
.'<a href="'.admin_url('admin.php?page='.$this->get_plugin_slug().'/settings.php&tab=free-widget-configurator&step=2').'">'.__('Please select another widget', 'review-widgets-for-tripadvisor').'.</a>';
...
/* translators: %s: Platform name */
$text = sprintf(__('There are no reviews on your %s platform.', 'review-widgets-for-tripadvisor'), ucfirst($this->getShortName()));

if ($this->is_review_download_in_progress()) {
$text = __('Your reviews are being downloaded.', 'review-widgets-for-tripadvisor') . ' ' . __('This process should only take a few minutes.', 'review-widgets-for-tripadvisor');
}
return $this->frontEndErrorForAdmins($text);
}
}
...
if (!$reviews) {
return self::get_alertbox('error', __('You do not have reviews with the current filters. <br />Change your filters if you would like to display reviews on your page!', 'review-widgets-for-tripadvisor'));
}
...
return $this->frontEndErrorForAdmins(__('Could not download the template for the widget.<br />Please reload the page.<br />If the problem persists, please write an email to support@trustindex.io.', 'review-widgets-for-tripadvisor') .'<br /><br />'. print_r($response, true));
}
...
if ($this->is_trustindex_connected()): ?>
<?php if ($ti_widgets): ?>
<h2><?php echo esc_html(__('Your saved widgets', 'review-widgets-for-tripadvisor')); ?></h2>
...
esc_html(__("You have no widget saved!", 'review-widgets-for-tripadvisor')) . " "
. "<a target='_blank' href='" . "https://admin.trustindex.io/" . "widget'>". esc_html(__("Let's go, create amazing widgets for free!", 'review-widgets-for-tripadvisor'))."</a>"
)); ?>
<?php endif; ?>
<?php else: ?>
<?php echo wp_kses_post(self::get_alertbox("warning",
esc_html(__("You have not set up your Trustindex account yet!", 'review-widgets-for-tripadvisor')) . " "
/* translators: %s: URL */
. wp_kses(sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'review-widgets-for-tripadvisor'), admin_url('admin.php?page='.$this->get_plugin_slug().'/settings.php&tab=advanced')))
)); ?>
<?php endif;
wp_die();
}
